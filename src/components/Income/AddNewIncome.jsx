...
export default function AddNewIncome({ isOpen, onClose, onSuccess }) {
  ...
  const createNewIncome = async (e) => {
    ...
    try {
      ...
      onSuccess?.();
      onClose();
      setFormData({ sourceName: "", monthlyAmount: "" });
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to create income source');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/80 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-xl w-full max-w-md p-6 relative animate-in fade-in duration-200">
        ...
        <h2 className="text-2xl font-semibold text-blue-400 mb-6">Add New Income Source</h2>

        <form onSubmit={createNewIncome} className="space-y-6">
          <div>
